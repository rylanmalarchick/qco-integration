...
\begin{abstract}
We present a comprehensive analysis of quantum circuit fidelity across the full compilation stack, from high-level gate optimization through pulse-level control. We connect a C++ circuit optimizer to a per-gate Lindblad master-equation fidelity model whose decoherence channels are cross-validated against \texttt{qiskit-dynamics} and whose absolute predictions are benchmarked against execution on real hardware. Across a campaign of 4{,}452 experiment runs over 371 benchmark circuits, gate cancellation provides the dominant improvement ($d = 1.66$, 72\% of circuits improved), while circuit size and pulse duration are the strongest negative predictors of process fidelity (input gates $r = -0.78$; pulse duration $r = -0.73$, $R^2 = 0.53$). A formal ablation study shows that pass ordering has no significant effect on two-qubit gate reduction (Kruskal--Wallis $p = 0.302$). Comparing against Qiskit transpilation levels, we show that two-qubit gate count---not total gate count---is the hardware-relevant metric: our optimizer attains superior two-qubit reduction on structured circuits (87.8\% on QFT, 100\% on QAOA) whereas Qiskit's larger total-gate reduction is dominated by single-qubit ($u_3$) consolidation. Finally, executing eight circuits on the IQM Resonance Garnet processor (8/8 jobs completed, job identifiers released) reveals that the model is a consistent upper bound: it preserves the relative difficulty ordering of circuits but overestimates absolute fidelity by a mean of 0.49, quantifying the error budget (crosstalk, leakage, readout) outside a $T_1$/$T_2$/depolarizing model. Our open-source framework enables reproducible, physically grounded benchmarking of quantum compilation pipelines.
\end{abstract}
...
\begin{figure}[t]
    ...
    \Description{Block diagram showing five pipeline stages: parse, optimize, route, pulse compile, and simulate, with arrows indicating data flow.}
\end{figure}
...
Figure~\ref{fig:pass} shows the comparative effectiveness of each optimization pass, and Table~\ref{tab:passes} provides the detailed breakdown. Gate cancellation emerges as the dominant pass, eliminating 3{,}780 gates across the corpus (run in isolation) with 72.0\% of circuits showing improvement.

\begin{figure}[t]
    \centering
    \includegraphics[width=\columnwidth]{figures/pass_effectiveness.pdf}
    \caption{Optimization pass effectiveness measured by net gate reduction. Error bars indicate standard deviation across the circuit corpus. Gate cancellation dominates, removing 3{,}780 gates.}
    \label{fig:pass}
    \Description{Bar chart comparing net gate reduction across four optimization passes, with gate cancellation showing the largest reduction.}
\end{figure}
...
\begin{figure}[t]
    ...
    \label{fig:waterfall}
    \Description{Waterfall chart showing stage-by-stage fidelity contributions, with two-qubit gate errors as the dominant source of fidelity loss.}
\end{figure}
...
\begin{figure}[t]
    ...
    \label{fig:scaling}
    \Description{Scatter plot of process fidelity versus qubit count grouped by circuit type, showing GHZ circuits maintaining higher fidelity than random circuits.}
\end{figure}
...
\begin{figure}[t]
    ...
    \Description{Grouped bar chart comparing two-qubit gate reduction percentages across circuit types for QCO and Qiskit compiler configurations.}
\end{figure}
...
\begin{figure}[t]
    ...
    \Description{Heatmap showing two-qubit gate reduction across circuit types and compiler configurations with color intensity indicating reduction magnitude.}
\end{figure}
...
The cancellation pass alone achieves a large effect ($d = 1.66$), reducing gate count by 9.1\% and two-qubit gate count by 2.6\% in isolation, with a corresponding increase in mean process fidelity from 0.525 (baseline) to 0.537. The remaining three passes produce negligible effects in isolation ($d < 0.2$ for all). Figure~\ref{fig:ablation-cumulative} shows the cumulative effect as passes are added sequentially.
...
\begin{figure}[t]
    ...
    \Description{Line chart showing cumulative gate reduction and fidelity as optimization passes are added sequentially.}
\end{figure}
...
\begin{figure}[t]
    ...
    \caption{Leave-one-out ablation results. Removing the cancellation pass eliminates essentially all optimization benefit (9.1\% marginal contribution, $d = 1.66$).}
    \label{fig:ablation-loo}
    \Description{Bar chart showing leave-one-out ablation results with the cancellation pass removal causing the largest performance drop.}
\end{figure}

Removing the cancellation pass eliminates essentially all optimization benefit (marginal contribution 9.1\%, $d = 1.66$, large effect), as shown in Figure~\ref{fig:ablation-loo}. Removing rotation merging, commutation, or identity elimination has negligible impact ($d < 0.02$), confirming that cancellation carries the pipeline.
...
\begin{enumerate}
    \item The cancellation pass provides essentially all of the optimization value, with a large effect size ($d = 1.66$). Other passes contribute marginally.
    ...
    \item Pass ordering has no statistically significant effect ($p = 0.302$), simplifying compiler design.
\end{enumerate}
...
Table~\ref{tab:hw-results} reports, for each circuit, the measured hardware fidelity alongside the validated Lindblad model's prediction. Figures~\ref{fig:hw} and~\ref{fig:hw-gap} visualize the comparison.
...
\begin{figure}[t]
    \centering
    \includegraphics[width=\columnwidth]{figures/fig_hardware_vs_sim.pdf}
    \caption{Validated simulation versus measured hardware fidelity per circuit on IQM Garnet. The model captures the relative ordering of circuits but overestimates absolute fidelity in every case.}
    \label{fig:hw}
    \Description{Grouped bar chart comparing simulated and measured hardware fidelity for eight circuits, with simulation consistently higher.}
\end{figure}

\begin{figure}[t]
    \centering
    \includegraphics[width=0.85\columnwidth]{figures/fig_hardware_gap.pdf}
    \caption{Measured hardware fidelity versus the model prediction. All points lie below the $y=x$ line (the model is optimistic), but the strong rank correlation shows the model preserves the relative difficulty ordering of circuits.}
    \label{fig:hw-gap}
    \Description{Scatter plot of measured versus simulated fidelity with a y equals x reference line; all points fall below the line.}
\end{figure}
...
The correlation $r = -0.73$ between pulse duration and process fidelity has a straightforward physical explanation. In the Lindblad framework (Equation~\ref{eq:lindblad}), the decoherence channels act continuously during pulse execution. For a circuit with total pulse duration $t_{\text{total}}$, the fidelity loss due to dephasing alone scales as:
\begin{equation}
...
\begin{itemize}
    ...
    \item The hardware subset (eight circuits) is smaller than the simulation campaign (371 circuits) due to access constraints, and was chosen to span circuit families rather than to isolate the optimization effect at fixed circuit structure.
...
\begin{itemize}
    ...
    \item An experiment runner (\texttt{experiments/\allowbreak run\_campaign.py}) that reproduces all eight experiment types with a single command.
    \item Statistical analysis scripts (\texttt{experiments/\allowbreak statistical\_analysis.py}) that generate all tables and figures from raw results.
    \item A benchmark compiler comparison script (\texttt{experiments/\allowbreak benchmark\_compilers.py}) for reproducing the Qiskit comparison.
...
The practical takeaway for quantum compiler developers is straightforward: implement gate cancellation first, because it provides nearly all the value. Report two-qubit gate counts for fair comparison, not total gates. And do not over-engineer pass ordering. The open-source framework enables reproducible benchmarking of quantum compilation pipelines with hardware validation capabilities.

Future work will expand hardware validation to additional platforms, integrate machine learning for adaptive pass selection, incorporate additional optimization passes for unstructured circuits, and extend the framework to fault-tolerant compilation pipelines.
